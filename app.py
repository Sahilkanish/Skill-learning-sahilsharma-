    ...
    if st.session_state.reset_mode:
        ...
        with t1:
            with st.form("login_form"):
                ...
                if st.form_submit_button("Login"):
                    conn = sqlite3.connect(DB_NAME)
                    user = conn.execute("SELECT * FROM users WHERE email=? AND password=?", (le, lpw)).fetchone()
                    conn.close()
                    if user:
        ...
        with t2:
            with st.form("s"):
                ...
                npw = st.text_input("Pass", type="password")
...
if st.session_state.logged_in:
    ...
    with tab_dash:
        ...
        if st.session_state.detection_data:
            ...
            st.markdown("---")
            st.subheader("📊 Graph and Analysis")
    ...
    with tab_stats:
        if is_admin:
            ...
            if st.button("📊 View Performance Metrics", use_container_width=True, type="primary"):
                ...
                m1.metric("Precision (B)", "85.8%")
                m2.metric("Recall (B)", "75.2%")
                m3.metric("Inference Speed", "15ms")
