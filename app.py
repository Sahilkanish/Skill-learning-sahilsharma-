...
if not st.session_state.logged_in:
    ...
    if st.session_state.reset_mode:
        ...
        if not st.session_state.otp_verified:
            ...
            if st.button("Send OTP"):
                ...
                if user:
                    ...
                    if send_otp_email(email_to_reset, otp):
                        st.session_state.generated_otp, st.session_state.target_email = otp, email_to_reset
            ...
            new_p, conf_p = st.text_input("New Password", type="password"), st.text_input("Confirm Password", type="password")
...
if uploaded_file:
    ...
    if coords:
        ...
        st.sidebar.success("📍 Location extracted!")
...
# --- STEP 3 SIDEBAR ADDED ---
st.sidebar.markdown("---")
st.sidebar.subheader("📂 Step 3: Check Reports")
st.sidebar.write("Historical Data per click karein aur purani reports check karein.")
...
with tab_dash:
    if uploaded_file:
        ...
        if st.button("🚀 Run AI Detection", type="primary", use_container_width=True):
            if yolo_model:
                ...
                st.session_state.res_img = res_plotted
                
                # Image save logic
                img_name = f"det_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
                img_path = os.path.join(UPLOAD_FOLDER, img_name)
                # Save processed image
                Image.fromarray(res_plotted).save(img_path) 
                st.session_state.last_img_path = img_path

                labels = results[0].boxes.cls.tolist()
                class_names = yolo_model.names
                p_count = sum(1 for label_id in labels if 'pothole' in class_names[label_id].lower())
                c_count = len(labels) - p_count
                
                st.session_state.p_count, st.session_state.c_count = p_count, c_count
    ...
    if st.session_state.get('detection_done'):
        ...
        with cr:
            ...
            st.markdown(f"<h3 style='color:{status_color};'>Status: {'DANGER' if p > 0 else 'SAFE'}</h3>", unsafe_allow_html=True)
            st.metric("🕳️ Potholes", p); st.metric("🚧 Cracks", c)
            if st.button("💾 Save Report to Database", use_container_width=True):
                conn = sqlite3.connect(DB_NAME)
                now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                conn.execute("INSERT INTO road_logs (timestamp, lat, lon, potholes, cracks, img_path) VALUES (?, ?, ?, ?, ?, ?)", 
                             (now, st.session_state.det_lat, st.session_state.det_lon, p, c, st.session_state.last_img_path))
                conn.commit(); conn.close()
                st.success("✅ Saved!")
...
with tab_hist:
    ...
    # Instruction Message
    st.info("💡 Niche diye gye drop down button se apni report ka selection kre")
    
    h_category = st.selectbox("Select Category:", ["All Reports", "Pothole Reports", "Crack Reports", "User Login Data"])
    conn = sqlite3.connect(DB_NAME)

    # Function to show images in history
    def display_with_images(df):
        if not df.empty:
            for index, row in df.iterrows():
                ...
                with col2:
                    ...
                    st.write(f"**Potholes:** {row['potholes']} | **Cracks:** {row['cracks']}")
                st.divider()
