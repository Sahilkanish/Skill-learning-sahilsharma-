    ...
    if st.session_state.get('detection_done'):
        ...
        with cr:
            ...
            if st.button("💾 Save Report to Database", use_container_width=True):
                ...
                img_save_path = f"saved_results/rep_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
                Image.fromarray(st.session_state.res_img).save(img_save_path)

                conn = sqlite3.connect(DB_NAME)
                now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                conn.execute("INSERT INTO road_logs (timestamp, lat, lon, potholes, cracks, image_path) VALUES (?, ?, ?, ?, ?, ?)", 
                             (now, st.session_state.det_lat, st.session_state.det_lon, p, c, img_save_path))
...
with tab_hist:
    ...
    img_column_config = {"image_path": st.column_config.ImageColumn("Preview", width="small")}
    ...
    if h_category == "All Reports":
        ...
        st.dataframe(df_p, column_config=img_column_config, use_container_width=True, hide_index=True)
        
        st.markdown("---")
        
        st.subheader("🚧 Crack Reports")
        df_c = pd.read_sql_query("SELECT * FROM road_logs WHERE cracks > 0 ORDER BY timestamp DESC", conn)
        st.dataframe(df_c, column_config=img_column_config, use_container_width=True, hide_index=True)
        ...
        st.dataframe(df, column_config=img_column_config, use_container_width=True, hide_index=True)
